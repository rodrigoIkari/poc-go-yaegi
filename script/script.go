...
func CalculateOperationStep(params map[string]string, input functions.Operation) (*functions.Operation, error) {
	fmt.Println("Iniciando cálculo de tabela PRICE")
	...
	fmt.Printf("Valor do financiamento: R$ %.2f\n", input.Amount)
	fmt.Printf("Quantidade de parcelas: %d\n", qtdeParcelas)
	fmt.Printf("Taxa de juros mensal: %.2f%%\n", taxaJuros*100)
	...
	fmt.Printf("Cálculo concluído! Total a pagar: R$ %.2f\n", input.TotalAmount)
